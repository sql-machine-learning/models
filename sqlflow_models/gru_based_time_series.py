...
class GRUBasedTimeSeriesModel(tf.keras.Model):
    ...
    def __init__(self,
        ...
        :param n_features: number of features in every time window.
            type n_features: int
        """
        super(GRUBasedTimeSeriesModel, self).__init__(name='GRU_TS_Model')
        # Common
        ...
        for unit in self.stack_units[:-1]:
            self.stack_layers.append(tf.keras.layers.GRU(unit, input_shape=(self.n_in, self.n_features), return_sequences=True))
        self.gru = tf.keras.layers.GRU(self.stack_units[-1], input_shape=(self.n_in, self.n_features))
