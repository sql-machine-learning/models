...
class TestStackedBiGRUClassifier(BaseTestCases.BaseTest):
    def setUp(self):
        ...
        emb = tf.feature_column.embedding_column(
            ...
            dimension=32)
        feature_columns = [emb]
        self.model = sqlflow_models.StackedBiGRUClassifier(feature_columns=feature_columns, stack_units=[64, 32])
        self.model_class = sqlflow_models.StackedBiGRUClassifier
